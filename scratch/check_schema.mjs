...
async function checkSchema() {
  try {
    ...
    const { data: locData, error: locError } = await supabase.from('locations').select('*').limit(1)
    if (locError) {
      console.log('Error fetching locations (might not exist):', locError.message)
    } else {
      console.log('Columns in locations:', Object.keys(locData[0] || {}))
    }
  } catch (err) {
